            ...
            <table class="w-full">
                    ...
                    <tr class="bg-gray-50 border-b border-gray-200">
                        <th class="px-5 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider group cursor-pointer hover:bg-gray-100 transition-colors" 
                            ...
                            <div class="flex items-center gap-2">
                                ...
                                <svg class="w-4 h-4 transition-all {{ $sortBy === 'nama_supplier' ? 'text-blue-600' : 'text-gray-300 group-hover:text-gray-500' }} {{ $sortBy === 'nama_supplier' && $sortDirection === 'desc' ? 'rotate-180' : '' }}" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M8 9l4-4 4 4m0 6l-4 4-4-4"/>
                                </svg>
                            </div>
                        </th>
                        <th class="px-5 py-4 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">Kontak</th>
                ...
                <tbody class="divide-y divide-gray-200">
                    ...
                    <tr class="hover:bg-blue-50/50 transition-colors">
                        <td class="px-5 py-4">
                            <span class="text-sm font-semibold text-gray-900">{{ $pemasok->nama_supplier }}</span>
                        </td>
                        <td class="px-5 py-4">
                            <span class="text-sm text-gray-600">{{ $pemasok->kontak ?? '-' }}</span>
                        </td>
                        <td class="px-5 py-4">
                            <span class="text-sm text-gray-600 truncate max-w-xs block">{{ $pemasok->alamat ?? '-' }}</span>
                        </td>
                        <td class="px-5 py-4">
                            <span class="text-sm text-gray-500">{{ $pemasok->catatan_termin_pembayaran ?? '-' }}</span>
                        </td>
                        <td class="px-5 py-4">
                            @if(auth()->user()->canModify())
                            <div class="flex items-center justify-center gap-1">
                                ...
                                <button onclick="event.preventDefault(); confirmDelete(@this, {{ $pemasok->id }})" 
                    ...
                    <tr>
                        <td colspan="5" class="px-5 py-12 text-center">
                            <div class="flex flex-col items-center justify-center text-gray-400">
                                ...
                                <p class="text-sm font-medium">Tidak ada data pemasok</p>
