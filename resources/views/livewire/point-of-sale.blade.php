                    ...
                    <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 hover:shadow-md transition-shadow">
                        ...
                        <div class="flex items-center justify-between mt-3">
                            <div class="flex items-center gap-1 bg-gray-100 rounded-lg p-1">
                                <button wire:click="updateQty({{ $index }}, {{ $item['jumlah'] - 1 }})" class="w-8 h-8 bg-white rounded-md flex items-center justify-center hover:bg-gray-50 text-gray-600 font-bold transition shadow-sm">−</button>
                                <input type="number" min="1" max="{{ $item['stok'] }}" 
                                    class="w-12 text-center text-sm font-semibold border-0 bg-transparent focus:outline-none focus:ring-0" 
                                    value="{{ $item['jumlah'] }}"
                                    wire:change="updateQty({{ $index }}, $event.target.value)"
                                    wire:keydown.enter="updateQty({{ $index }}, $event.target.value)">
                                <button wire:click="updateQty({{ $index }}, {{ $item['jumlah'] + 1 }})" class="w-8 h-8 bg-white rounded-md flex items-center justify-center hover:bg-gray-50 text-gray-600 font-bold transition shadow-sm">+</button>
                            </div>
                            <p class="font-bold text-sky-600">Rp {{ number_format($item['subtotal'], 0, ',', '.') }}</p>
                        </div>
                    </div>
                @empty
